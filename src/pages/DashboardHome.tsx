...
export default function DashboardHome() {
  ...
  const stageSpread: CampaignStage[] = ['pitching', 'active', 'review', 'complete'];

  const safeAthletes = filteredAthletes ?? [];
  const safeCampaigns = filteredCampaigns ?? [];
  const safeTasks = filteredTasks ?? [];
  const safeEvents = eventsNextWeek ?? [];
  const safeUpcoming = upcoming14Days ?? [];
  const myTasks = safeTasks
    .slice()
    .filter((task) => task?.dueDate instanceof Date && !Number.isNaN(task.dueDate.getTime()))
    .sort((a, b) => a.dueDate.getTime() - b.dueDate.getTime());
  const sportBreakdown = safeAthletes.reduce<Record<string, number>>((acc, athlete) => {
    ...
    <AppLayout>
      <div className="max-w-[1600px] mx-auto space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ['Athletes under management', kpis?.athletesUnderManagement ?? 0],
            ['Active campaign value', kpis?.activeCampaignValue ?? '$0'],
            ['Athlete earnings YTD', kpis?.athleteEarningsYtd ?? '$0'],
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
          <div className="xl:col-span-8 space-y-4">
            <section className="p-4 rounded-xl border border-border bg-card">
              ...
              <h2 className="font-semibold">Campaign pipeline</h2>
                <Button size="sm" variant="outline" onClick={() => openModal('campaign')}>Add campaign</Button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                {stageSpread.map((stage) => {
                  const stageCampaigns = safeCampaigns.filter((campaign) => campaign.stage === stage && !campaign.archived);
              ...
              <div className="space-y-2 max-h-[320px] overflow-auto pr-1">
                {safeCampaigns.filter((campaign) => !campaign.archived).map((campaign) => {
                  const athlete = safeAthletes.find((entry) => entry.id === campaign.athleteId);
                  return (
                ...
                {safeCampaigns.filter((campaign) => !campaign.archived).length === 0 && (
                  <p className="text-sm text-muted-foreground">No campaigns in pipeline.</p>
                )}
              </div>
            </section>
          ...
          <div className="xl:col-span-4 space-y-4">
            <section className="p-4 rounded-xl border border-border bg-card">
              ...
              <div className="space-y-2">
                {stageSpread.map((stage) => {
                  const stageCampaigns = safeCampaigns.filter((campaign) => campaign.stage === stage && !campaign.archived);
                  const stageValue = stageCampaigns.reduce((sum, campaign) => sum + (campaign.value || 0), 0);
                  return (
                    <div key={stage} className="rounded-lg border border-border px-3 py-2">
                      <p className="text-sm font-medium">{stageLabels[stage]}</p>
                      <p className="text-xs text-muted-foreground">
                        {stageCampaigns.length} campaigns • {formatCurrency(stageValue)}
                      </p>
                    </div>
                  );
                })}
            ...
            <section className="p-4 rounded-xl border border-border bg-card">
              <h2 className="font-semibold mb-3">Roster by sport</h2>
              <div className="space-y-2">
                ...
                {Object.keys(sportBreakdown).length === 0 && <p className="text-sm text-muted-foreground">No athletes found.</p>}
              </div>
            </section>
            ...
            <section className="p-4 rounded-xl border border-border bg-card">
              ...
              <div className="space-y-2 max-h-[220px] overflow-auto pr-1">
                ...
                {safeUpcoming.length === 0 && <p className="text-sm text-muted-foreground">No upcoming deliverables.</p>}
              </div>
            </section>
            ...
            <section className="p-4 rounded-xl border border-border bg-card">
              ...
              <div className="space-y-2">
                ...
                {safeEvents.length === 0 && <p className="text-sm text-muted-foreground">No calendar events.</p>}
              </div>
            </section>
