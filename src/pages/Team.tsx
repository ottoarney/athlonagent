...
function statusClass(status: CapacityStatus) {
  if (status === 'Wide open') return 'bg-[#01FB64]/20 text-[#111111] border-[#01FB64]/40';
  if (status === 'Balanced') return 'bg-slate-50 text-slate-700 border-slate-200';
  return 'bg-[#FBE101]/20 text-[#111111] border-[#FBE101]/40';
}
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedDepartment === department
                    ...
                    : 'border-border hover:border-[#01FB64]/50',
                )}
              >
            ...
            {whoIsFreeRows.map((teammate, index) => (
              <article key={teammate.id} className="rounded-xl border border-border bg-white p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {index === 0 ? (
                    <span className="rounded-full border border-[#01FB64]/40 bg-[#01FB64]/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#111111]">
                      Top Match
                    </span>
                  ) : null}
                </div>
